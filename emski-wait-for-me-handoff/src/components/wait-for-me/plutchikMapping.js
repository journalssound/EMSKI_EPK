...
function emotionFlow(v, seed = 0) {
  ...
  // Each emotion gets a DISTINCT gesture grounded in its embodied quality.
  // All originate from the self (canvas center, ox=0.5, oy=0.5) — what
  // differs is HOW the energy moves from that origin.
  switch (name) {
    case "joy":
      // Bloom — smooth radial bloom outward, 360° even spread
      return { mode: "bloom", strength: weight * 0.55 * strMul, ox: 0.5, oy: 0.5, rotation, curlMul };
    case "sadness":
      // Settle — emerges from self, sinks down, pools at bottom
      return { mode: "settle-down", strength: weight * 0.55 * strMul, ox: 0.5, oy: 0.5, rotation, curlMul };
    case "anger":
      // Jagged burst — 6 sharp angled rays, fast, short
      return { mode: "jagged-burst", strength: weight * 0.6 * strMul, ox: 0.5, oy: 0.5, rotation, curlMul };
    case "fear":
      // Scatter — outward burst that disperses, fleeing
      return { mode: "scatter", strength: weight * 0.5 * strMul, ox: 0.5, oy: 0.5, rotation, curlMul };
    case "surprise":
      // Flash — explosive 360° burst, very fast, very short
      return { mode: "flash", strength: weight * 0.55 * strMul, ox: 0.5, oy: 0.5, rotation, curlMul };
    case "disgust":
      // Recoil — slow outward pulsing wave, pushing away
      return { mode: "recoil", strength: weight * 0.5 * strMul, ox: 0.5, oy: 0.5, rotation, curlMul };
    case "trust":
      // Orbital — slow rotation around self, steady and grounded
      return { mode: "rotational", strength: weight * 0.35 * strMul, ox: 0.5, oy: 0.5, rotation, curlMul };
    case "anticipation":
      // Rising — reaches upward from self, building tension
      return { mode: "rising", strength: weight * 0.5 * strMul, ox: 0.5, oy: 0.5, rotation, curlMul };
    default:
      return null;
  }
}
