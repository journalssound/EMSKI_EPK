...
varying float v_alpha;
void main() {
  // u_alpha is the global brightness, v_alpha is the per-particle hero/wash
  // multiplier. Multiplied so wash particles recede and gesture particles
  // read as the foreground stroke.
  gl_FragColor = vec4(u_color, u_alpha * v_alpha);
}
`;
...
const FieldCanvas = forwardRef(function FieldCanvas(
  ...
  useEffect(() => {
    ...
    const drawAlphaBuf = new Float32Array(N * 2); // alpha per line vertex
    ...
    function spawnParticle(i, fresh = false) {
      ...
      if (flow) {
        ...
        // All gesture spawns originate near canvas center (the SELF). The
        // per-emotion direction comes from initial velocity + continued
        // force during tick — emotions emerge FROM the self, they don't
        // arrive at the self from outside the frame.
        switch (flow.mode) {
          case "bloom":
          case "radial-out": {
            // JOY — smooth bloom, even 360° outward spread from center
          ...
          case "settle-down":
          case "gravity-down": {
            // SAD — same outward bloom from center as joy. The DIFFERENCE
            // is the line SHAPE: gentle downward sag during travel makes
            // each line bow/droop rather than going straight. Line shape
            // is the emotional content, not direction.
            const a0 = Math.random() * Math.PI * 2;
            x = ox + Math.cos(a0) * jitter;
            y = oy + Math.sin(a0) * jitter;
            vx0 = Math.cos(a0) * 0.5;
          ...
          case "jagged-burst": {
            // ANGER — 6 sharp rays, fast outward bursts, NOT smooth 360°
            const numRays = 6;
            const rayIdx = Math.floor(Math.random() * numRays);
            const baseAngle = (rayIdx / numRays) * Math.PI * 2;
            const angleJitter = (Math.random() - 0.5) * 0.30; // narrow within ray
            const a0 = baseAngle + angleJitter;
            x = ox + Math.cos(a0) * jitter * 0.3; // tighter origin
            y = oy + Math.sin(a0) * jitter * 0.3;
            const speed = 0.8; // fast
            vx0 = Math.cos(a0) * speed;
            vy0 = Math.sin(a0) * speed;
            break;
          }
          case "scatter": {
            // FEAR — same outward bloom as joy. Line SHAPE: trembling /
            // wavering. Perpendicular oscillation during travel makes each
            // line shake like nervous hands.
            const a0 = Math.random() * Math.PI * 2;
            x = ox + Math.cos(a0) * jitter;
            y = oy + Math.sin(a0) * jitter;
            vx0 = Math.cos(a0) * 0.45;
            vy0 = Math.sin(a0) * 0.45;
            break;
          }
          case "flash": {
            // SURPRISE — explosive 360° flash, very fast initial velocity
            const a0 = Math.random() * Math.PI * 2;
            x = ox + Math.cos(a0) * jitter * 0.2; // very tight origin
            y = oy + Math.sin(a0) * jitter * 0.2;
            const speed = 1.1; // very fast
            vx0 = Math.cos(a0) * speed;
            vy0 = Math.sin(a0) * speed;
            break;
          }
          case "recoil": {
            // DISGUST — slow outward, pushed away in waves
            const a0 = Math.random() * Math.PI * 2;
            x = ox + Math.cos(a0) * jitter * 0.5;
            y = oy + Math.sin(a0) * jitter * 0.5;
            const speed = 0.22; // slow
            vx0 = Math.cos(a0) * speed;
            vy0 = Math.sin(a0) * speed;
            break;
          }
          case "rising": {
            // ANTICIPATION — mirror of sad. Outward bloom from center
            // (same as joy), but lines bow UPWARD instead of down.
            const a0 = Math.random() * Math.PI * 2;
            x = ox + Math.cos(a0) * jitter;
            y = oy + Math.sin(a0) * jitter;
            vx0 = Math.cos(a0) * 0.5;
          ...
          case "rotational": {
            // TRUST — slow orbital around self (steady, grounded)
    ...
    const drawAlphaBufferGL = regl.buffer({ data: drawAlphaBuf, usage: "dynamic", length: N * 2 * 4 });
    ...
    const drawLines = regl({
      vert: LINE_VERT,
      frag: LINE_FRAG,
      attributes: { a_pos: drawLineBufferGL, a_alpha: drawAlphaBufferGL },
      uniforms: {
        u_viewport: () => [width, height],
        u_color: regl.prop("color"),
        u_alpha: regl.prop("alpha"),
    ...
    const tick = (ts) => {
      ...
      if (!lastTs) lastTs = ts;
      // 0.6 = global time-scale; the whole simulation runs at 60% speed.
      // Scales both position updates AND force application so the trajectory
      // shapes stay identical, just slower.
      const dt = (Math.min(48, ts - lastTs) / 16.67) * 0.6;
      lastTs = ts;
      ...
      const flowOx = flow && flow.ox != null ? flow.ox * width : width * 0.5;
      const flowOy = flow && flow.oy != null ? flow.oy * height : height * 0.5;
        ...
        // ── Directional flow + jitter ──
        {
          if (flow) {
            ...
            if (flow.mode === "settle-down" || flow.mode === "gravity-down") {
              // sad — very weak downward force. Strong enough to bow the
              // outward-radiating lines into a sagging shape over their
              // length, weak enough that lines don't get yanked into
              // straight downward streaks.
              baseFy = flowS * 0.30;
            } else if (flow.mode === "gravity-up") {
              baseFy = -flowS;
            } else if (flow.mode === "rising") {
              // anticipation — weak upward force = lines bow UPWARD over
              // their length (mirror of sad's downward droop).
              baseFy = -flowS * 0.30;
            } else if (flow.mode === "horizontal-right") {
              baseFx = flowS;
              ...
              // SURPRISE — strong extra drag mid-flight so lines TAPER
              // quickly. Particles burst fast, then halt. Lines start bold
              // and peter out — flash-photography shock.
              const flashDrag = Math.pow(0.94, dt);
              vxNew *= flashDrag;
              vyNew *= flashDrag;
